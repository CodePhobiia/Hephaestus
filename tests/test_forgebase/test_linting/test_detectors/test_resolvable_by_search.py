...
async def _create_claim_with_support(
    uow, vault_id, page_id, statement, source_id, clock, id_gen,
    status=ClaimStatus.SUPPORTED, num_supports=1,
):
...
async def test_detects_inferred_claim(env):
    ...
    async with uow2:
        ...
        resolvable = [
            f for f in findings
            if f.category == FindingCategory.RESOLVABLE_BY_SEARCH
            and f.claim_id == claim_id
        ]
